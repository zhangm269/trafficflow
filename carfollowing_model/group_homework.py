...
x_j=[40]    #用于记录第(i-1)辆车(前车)的行驶距离，这里初始化多余！
v_j=[0]     #用于记录第(i-1)辆车的速度
a_j=[0]     #用于记录第(i-1)辆车的加速度

#以下四个变量用于计算过程中的用于记录某一时刻的“时刻、加速度、速度、行驶距离、初始车头间距”
t0=0
a0=0
v0=0
x0=0
...
time_gap=0.1    #计数间隔
T=0.5           #反应时间
r=1/T           #反应强度（用于计算减速度）ppt第20页
v_max=36        #第i辆车的最大行驶速度，即理想速度
v1=24            #前车速度，场景三用到
a=5             #场景一试验车用到的加速度大小
a2=-3            #试验车制动加速度，场景二用到？为什么是加速度！！！！
a3=-3           #前车减速度，场景三用到
a4=2            #前车加速度，场景四用到
...
for i in range(int(100/time_gap+1)):#场景一，前100秒启动、加速自由流行驶过程
    ...
    x_j.append(x0+h)#往第(i-1)辆车行驶距离数组中添加记录？这里有没有必要！！！！
...
for i in range(int(100/time_gap+1),int(200/time_gap+1)):#场景二，被抢道、跟驰过程（100秒）
    ...
    if flag==0:
        if t2<T:                    #驾驶员反应车辆i-1需要T时间
            ...
            v0=v_max
            h=h+(v_j2-v0)*time_gap # 结束后h变为22m
        else:                       #反应结束，开始刹车
            # 加入距离过近的修正方案，如果距离过近，就会退回200米之外，即减速再加速的过程正好让i车与i-1车距离200米
            if h<(safety-(v_j2*v0*2-v0**2)*(24/(v_j2+v0)-1/2)/a):
                ...
                flag=1
    else:                       #减速结束，进入加速过程，让i车与i-1车保持匀速且距离为200米
...
for i in range(int(300/time_gap+2),int(400/time_gap+1)):#场景四，随行过程（100秒）
    ...
    t_need = 36/a4   # 用于计算加速时间

    if t4 > t_need:
        accelerate = 0

    if accelerate:
        v1 = v1 + a4 * time_gap #加速

    distance = x_j[-1] -  x_i[-1]  #车距
    if distance < safety:  # 车距小于安全距离需要改变行车状况
        follow = 1
    else:
        follow = 0

    x1 = x1 + v1*time_gap   #更新i-1车位置
    a = r * (v0 - v1)  # p26 model


    if follow==1:                #跟车状况
        t_reflect = t_reflect+time_gap

        if t_reflect >= T:       # 达到反应时间，驾驶员开始反应
            a0 = a
...
t31=0
v1=36
for i in range(int(400/time_gap+2),int(420/time_gap+1)):#场景五，靠近过程（20秒）
    t0 = t[i]  # 记录i-1车的速度
    t31 = t31 + time_gap  # 用于计算反应时间
    if v1 > 0:  # 前车动作，车辆做匀减速运动直至速度减至0
        a3 = -3
...
ax1=plt.subplot(221)
plt.plot(x,y0,color='r',label='x_i')
plt.plot(x,y,color='g',label='x_j')
ax1.set_xlabel('t(s)')
ax1.set_ylabel('x(m)')
plt.legend()

ax2=plt.subplot(222)
plt.plot(x,y1,color='g',label='v')
ax2.set_xlabel('t(s)')
ax2.set_ylabel('v(m/s)')


ax3=plt.subplot(223)
plt.plot(x,y2,color='b',label='a')
ax3.set_xlabel('t(s)')
ax3.set_ylabel('a(m/s^2)')

ax4=plt.subplot(224)
plt.plot(x,y3,color='b',label='a')
ax4.set_xlabel('t(s)')
ax4.set_ylabel('h(m)')
plt.tight_layout()
#plt.legend()
