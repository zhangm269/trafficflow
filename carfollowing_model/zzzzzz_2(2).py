    ...
    def run(self):
        ...
        for i in range(int(300/time_gap+2),int(400/time_gap+1)):#场景四，随行过程（100秒）
            ...
            if(v_41<=v_max and v0<=v_max):
                a_41=2
                x_41=x_41+v_41*time_gap+0.5*a_41*time_gap**2
                ...
                if(t0-t[int(300/time_gap+2)]<=T):
                    ...
                    a0=r*(v_41-v0)
                    x0=x0+v0*time_gap+0.5*a0*time_gap**2
                    v0=v0+a0*time_gap
        ...
        t_finally=np.linspace(start_time,t0,len(a_i))#记录已完成过程的时间，注意"t0"一定要更新
        ...
        for i in range(len(x_i)):
            jian_ju=x_j[i]-x_i[i]-l_j
            if(jian_ju<0):#判断是否撞车
                print("在第%.2f秒发生撞车，此时的车头间距为%.2f"%((i*time_gap),jian_ju))
                y3.append(jian_ju)
            else:#如果没有撞车，往车头间距数组中添加记录
                y3.append(jian_ju)
        return x,y0,y00,y1,y11,y2,y22,y3



def drow_dui_bi(x_data,y_data1,y_data2,color1='g',color2='r',label1='x_i',label2='x_j',xlabel='t(s)',ylabel='x(m)'):
    ax1=plt.subplot()
    plt.plot(x_data,y_data1,color=color1,label=label1)
    plt.plot(x_data,y_data2,color=color2,label=label2)
...
def drow(x_data,y_data,color='g',label='h',xlabel='t(s)',ylabel='h(m)'):
    ax1=plt.subplot()
...
if __name__=='__main__':
    print("请输入0或者1选择不同的操作，0表示要自己输入参数，1表示使用预设值。")
    flag=input("请输入0或者1，选择不同的操作：")
    if(flag=='0'):
        T=input("请输入T的值：")
        r=input("请输入r的值：")
        linear = Linear(float(T),float(r))
        x,y0,y00,y1,y11,y2,y22,y3=linear.run()
        drow_dui_bi(x,y0,y00)
        drow_dui_bi(x,y1,y11)
        drow_dui_bi(x,y2,y22)
        drow(x,y3)
        pass
    else:           
        ax1=plt.subplot()
        for i in range(5,10):
            linear = Linear(i/10,20/i)
            x,y0,y00,y1,y11,y2,y22,y3=linear.run()
            label="T="+str(i/10)+','+"r="+str(20/i)
            plt.plot(x,y0,label=label)
            #plt.annotate(label,xy=(0,i),xytext=(-2,22),arrowprops={'headwidth':10,'facecolor':'r'})
        plt.title('x-t')
        plt.xlabel('t(s)')
        plt.ylabel('x(m)')
        plt.legend()
        ...
        for i in range(5,10):
            ...
            #plt.annotate(label,xy=(0,1),xytext=(-2,22),arrowprops={'headwidth':10,'facecolor':'r'})
        plt.title('h-t')
        plt.xlabel('t(s)')
        plt.ylabel('h(m)')
        plt.legend()
